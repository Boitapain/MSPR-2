...
def update_users(api_url, users_edited):
    data_json = users_edited.to_json(orient="records")
    response = requests.put(
        f"{api_url}/update_users",
        json={"users": data_json},
        headers={"Content-Type": "application/json"}
    )
    return response

def show_progress_bar(t):
...
def show_admin_editor(users, t, api_url):
    ...
    with cols[1]:
        ...
        if st.button(t['update_button'], help=t['help_messages']['update_button']):
            ...
            if users_edited.isnull().values.any():
                ...
                response = update_users(api_url, users_edited)
...
def show_readonly_view(users, t):
